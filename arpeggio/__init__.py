...
class OrderedChoice(Sequence):
    ...
    def _parse(self, parser, out_weakly_failed_rules: typing.Optional[typing.List] = None):
        ...
        if not match:
            ...
            parser.nm = NoMatch(failed_rules, c_pos, parser)
            raise parser.nm
...
class Optional(Repetition):
    ...
    def _parse(self, parser, out_weakly_failed_rules: typing.Optional[typing.List] = None):
        ...
        try:
            ...
            parser.position = c_pos  # Backtracking
            weakly_failed_rules.append(no_match_exc_)
        if out_weakly_failed_rules is not None:
            out_weakly_failed_rules.extend(weakly_failed_rules)
...
class UnorderedGroup(Repetition):
    ...
    def _parse(
        ...
        if not match:
            parser.nm = NoMatch(failed_nodes.values(), failed_position, parser)
        ...
        try:
            for e in self.nodes:
                try:
                    ...
                    parser.position = c_pos
                    weakly_failed_rules.append(NoMatch([self], c_pos, parser))
                    return None

            parser.nm = NoMatch([self], c_pos, parser)
            parser.position = c_pos
            raise parser.nm
        finally:
