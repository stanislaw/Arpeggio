...
def test_zero_or_more_with_separator():
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("a,a ,a,")
    assert (
        "Expected 'a' at position (1, 8) or EOF at position (1, 7) => 'a,a ,a,*'."
    ) == str(e.value)
...
def test_one_or_more_with_optional_separator():
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("a a, b")
    assert (
       "Expected 'a' at position (1, 6) or 'b' at position (1, 4) => 'a a, *b'."
    ) == str(e.value)
...
def test_not():
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("ab")
    assert (
       "Expected not('b') at position (1, 2) => 'a*b'."
    ) == str(e.value)
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("acb")
    assert (
       "Expected not('b') at position (1, 2) or EOF at position (1, 3) => 'ac*b'."
    ) == str(e.value)
